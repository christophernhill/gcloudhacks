...
#  docker run --rm --volumes-from ${GCP_SESSION_NAME} -it ${GCP_SESSION_NAME} gcloud auth login
#  docker run --rm --volumes-from ${GCP_SESSION_NAME} -it ${GCP_SESSION_NAME} gcloud config set core/project   ${GCP_PROJECT}
#  docker run --rm --volumes-from ${GCP_SESSION_NAME} -it ${GCP_SESSION_NAME} gcloud config set compute/region ${GCP_REGION}
#  docker run --rm --volumes-from ${GCP_SESSION_NAME} -it ${GCP_SESSION_NAME} gcloud config set compute/zone   ${GCP_ZONE}
#
# To shut down
